...
| `attendance_checked_in.sql` | `attendance.checked_in`(출결 ON만 대관료 정산 목록) |
...
| `/my-page` | Elo·본인 티어 마크·**display_name**·우측 **Tier System**(4티어 마크+라벨, 임시 티어 각주)·통산·최근 10게임 도넛·파트너 Top3·**나의 전적**(승인 경기, **`ApprovedMatchResultRow`** = 결과 조회와 동일 카드) |
...
| `/ranking` | Elo 랭킹·티어 필터·`#`·선수(마크+이름)·Elo·**승/패** 중앙 정렬 · **매칭추천** → `/match-suggest` |
| `/match-suggest` | 다가오는 open 매치·참석 4명 이상·본인 포함 4인 구성(동반 선택 반영, 부족 시 참석 풀 랜덤)·**Snake** 팀 제안(`lib/teamMatching.ts`) |
| `/admin/*` | **운영진(`role=staff`)**만 — `AdminGuard` |
| `/admin/roles` | 회원/운영진 역할 지정 |
| `/admin/players` | 선수 CRUD |
| `/admin/matchs` | 매치 일정·출석 관리 (팀 제안은 회원 **랭킹 → 매칭추천** `/match-suggest`) |
...
**상단 네비(`MainNav`)**: **MY** + 한글 메뉴 **2줄** 라벨(참여/신청·매치/조회 등), 로그인 시 6칩·`flex-1` 균등 너비·한 줄.
...
- `components/TierMarkImage.tsx` — 티어 로고 원형 마크
- `components/ApprovedMatchResultRow.tsx` — 승인 경기 한 건 표시(결과 조회·My **나의 전적** 공통)
- `lib/myPageData.ts` — 통산·최근 10게임·파트너 Top3·`fetchMyApprovedMatchLines`
- `lib/matches.ts` — `listApprovedMatches({ session_id?, player_id?, limit? })` 등
